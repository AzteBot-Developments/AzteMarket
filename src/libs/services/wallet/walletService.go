...
func (s WalletService) SendFundsWithUserIds(senderUserId string, receiverUserId string, funds float64) (float64, error) {
	...
	// Validation
	if funds > senderWallet.Funds {
		return -1, fmt.Errorf("sender `%s` cannot transfer more AzteCoins than they own (available: `🪙 %.2f`)", senderUserId, senderWallet.Funds)
	}
	...
	// ensure that receiver has a wallet
	receiverWalletId, err := s.WalletsRepository.GetWalletIdForUser(receiverUserId)
	if err != nil {
